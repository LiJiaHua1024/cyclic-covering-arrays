...
6. **[T8] $C_{19}$ 显式 13 行见证与奇数环跃迁阈值（最新突破）**
   - 构造并验证了 $C_{19}$ 的 13 行显式覆盖阵列，$N(19) \le 13 \implies 12 \le LP(19) \le N(19) \le 13$。
   - 严格证明了奇数环上使最优行数首次达到 14 行的最小环长必须满足 $n^*_{\text{odd}} \ge 21$。
...
- `python verifiers/verify_c19.py`：独立复核 $C_{19}$ 显式 13 行见证（665 状态覆盖）与 209 个等号行 11 轨道分类。
